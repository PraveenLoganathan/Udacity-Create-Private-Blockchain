...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            // Calculate and assign current block hash

            block.hash = SHA256(JSON.stringify(block)).toString();

            // Validate whether current block has been tampered with

            let validateBlock = await block.validate();
            if (validateBlock == false) {
                reject(Error("This block may have been tampered with"));
            }

            // Validate whether the chain has been tampered with
            ...
            // If no mischievous activity - push block into the chain
        ...
        return new Promise(async (resolve, reject) => {
            ...
            self.chain.forEach(async function (block, height) {
                ...
                if (block.previousBlockHash != previousBlockHash) {
                    errorlog.push("Previous block hash is invalid!")
                }
